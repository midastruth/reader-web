...
export const StatefulReaderWrapper = ({ profile, plugins, ...props }: ReaderComponentProps) => {
  ...
  const theme = profile === "audio" ? themeObject.audio : (profile === "epub" ? (isFXL ? themeObject.fxl : themeObject.reflow) : ThThemeKeys.light);
  ...
  useTheming<ThemeKeyType>({
    ...
    coverUrl,
    autoThemeSource: profile === "audio" ? "cover" : "system",
    initProps: {
  ...
  switch (profile) {
    ...
    case "audio":
      return <Suspense><StatefulPlayer { ...props } coverUrl={ coverUrl } /></Suspense>;
    case "webPub":
    default:
